...
export const getEvent = async (eventId: string) => {
  ...
  throw new Error(
    `Response status ${response.status}: It could not get event id ${eventId}`
  )
}

export const getEvents = async (userId: string) => {
  const response = await fetch(
    `${endpoint}/events?hostId=${userId}&pageSize=100&page=1`
  )
  if (response.status === 200) {
...
export const subscribeToEvent = async (eventId: string, guestId: string) => {
  ...
  throw new Error(
    `Response status ${response.status}: It could not subscribe to event`
  )
}
